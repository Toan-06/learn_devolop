...
console.log("🚀 Generating 20 Meticulous Samples Per Subject x 5 Subjects = 100 Samples Per Grade (1,200 Total)...");

// Helper for generating deep math solution (20 variants per grade)
...
// Helper for generating deep Literature essays (20 full essays per grade)
function genLitSample(grade, index) {
  const topics = [
    'Phân tích bài thơ "Tây Tiến" (Quang Dũng) — Tượng đài bi tráng người lính Hà Thành',
    'Phân tích bài thơ "Sóng" (Xuân Quỳnh) — Khát vọng tình yêu vĩnh hằng của người phụ nữ',
    ...
    'Bài văn đạt điểm 9.5+ Thi THPTQG: Tổng hợp phân tích tác phẩm & Nghị luận xã hội'
  ];
  const topic = topics[(index - 1) % topics.length];
...
<div style="background:rgba(0,242,254,0.06);border:1.5px solid rgba(0,242,254,0.3);border-radius:16px;padding:16px;margin-bottom:14px;">
  <b style="color:#00f2fe;font-size:1.05rem;">1. Bố Cục Chi Tiết 3 Phần & Hệ Thống Luận Điểm 9.5+</b>
  <ul style="margin:8px 0 0 18px;padding:0;line-height:1.8;color:#cbd5e1;">
    ...
    <li><b>Thân bài (3 Luận điểm):</b> Phân tích chi tiết từng nét nghệ thuật, hình tượng nhân vật và giá trị nhân đạo.</li>
    <li><b>Kết bài:</b> Đánh giá tầm vóc tác phẩm và bài học cuộc sống cho học sinh Lớp ${grade}.</li>
  </ul>
</div>`,
    full_essay: `<b>MỞ BÀI:</b>
Nhà văn Maxim Gorky từng khẳng định: <i>"Văn học làm cho con người thêm phong phú, tạo khả năng cho con người lớn lên, hiểu được con người đối với con người."</i> Trong chương trình Ngữ Văn Lớp ${grade}, chủ đề <b>"${topic}"</b> hiện lên như một kiệt tác ngôn từ rực rỡ. Tác phẩm không chỉ thu hút người đọc bởi bút pháp miêu tả bậc thầy mà còn bởi những thông điệp nhân sinh sâu sắc được gửi gắm qua từng trang viết.
...
// Helper for generating English samples (20 per grade)
function genEngSample(grade, index) {
  const topics = [
    ...
    'Inversion with Negative Adverbials (Not only, Hardley, No sooner)',
...
// Helper for generating Science samples (20 per grade)
...
// Helper for generating Social samples (20 per grade)
function genSocSample(grade, index) {
  const topics = [
    ...
    'Lịch sử văn hóa & Các di sản văn hóa thế giới tại Việt Nam',
     me = 'Toàn cầu hóa & Hội nhập kinh tế quốc tế',
...
// Build 100 samples (20 per subject x 5 subjects) for a given grade
function buildGrade100Bank(grade) {
  const math = [];
  const literature = [];
  const english = [];
  const science = [];
...
const g1_5 = {};
for (let g = 1; g <= 5; g++) {
  g1_5[g] = buildGrade100Bank(g);
}
fs.writeFileSync(
  path.join(__dirname, '..', 'assets', 'samples_grade1_5.js'),
  `// ===== BÀI MẪU SIÊU CHI TIẾT CHUẨN 20 BÀI/MÔN (100 BÀI/LỚP) LỚP 1 - 5 =====\n` +
...
console.log("✅ Written 20 samples/subject to samples_grade1_5.js!");

// 2. Write Grades 6-10 files
for (let g = 6; g <= 10; g++) {
  const bank = buildGrade100Bank(g);
  fs.writeFileSync(
    path.join(__dirname, '..', 'assets', `samples_grade${g}.js`),
    `// ===== BÀI MẪU SIÊU CHI TIẾT CHUẨN 20 BÀI/MÔN (100 BÀI/LỚP) LỚP ${g} =====\n` +
    `window.SAMPLES_GRADE_${g} = ${JSON.stringify(bank, null, 2)};\n`,
    'utf8'
  );
  console.log(`✅ Written 20 samples/subject to samples_grade${g}.js!`);
}

// 3. Write Grades 11-12 file
const g11 = buildGrade100Bank(11);
const g12 = buildGrade100Bank(12);
fs.writeFileSync(
  path.join(__dirname, '..', 'assets', 'samples_grade11_12.js'),
  `// ===== BÀI MẪU SIÊU CHI TIẾT CHUẨN 20 BÀI/MÔN (100 BÀI/LỚP) LỚP 11 & 12 =====\n` +
  `window.SAMPLES_GRADE_11 = ${JSON.stringify(g11, null, 2)};\n` +
  `window.SAMPLES_GRADE_12 = ${JSON.stringify(g12, null, 2)};\n`,
  'utf8'
);
console.log("✅ Written 20 samples/subject to samples_grade11_12.js!");

console.log("🎉 ALL 12 GRADES NOW HAVE EXACTLY 20 SAMPLES PER SUBJECT (1,200 TOTAL DETAILED SAMPLES)!");
