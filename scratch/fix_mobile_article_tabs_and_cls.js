...
console.log("⚡ Fixing Mobile Article Length (Section Tabs System) & Eliminating Layout Shifts (CLS) / Slow Loading...");

// 1. Update index.html: Add samples_grade6.js, add defer, add CLS prevention styles
const indexPath = path.join(__dirname, '..', 'index.html');
let indexHtml = fs.readFileSync(indexPath, 'utf8');

// Ensure samples_grade6.js is present and scripts have defer
const scriptBlock = `    <script defer src="assets/samples_grade1_5.js"></script>
    <script defer src="assets/samples_grade6.js"></script>
    <script defer src="assets/samples_grade7.js"></script>
    <script defer src="assets/samples_grade8.js"></script>
    <script defer src="assets/samples_grade9.js"></script>
    <script defer src="assets/samples_grade10.js"></script>
    <script defer src="assets/samples_grade11_12.js"></script>
    <script defer src="assets/sample_viewer.js"></script>
    <script defer src="assets/auth.js"></script>`;

if (indexHtml.includes('<script src="assets/samples_grade1_5.js"></script>')) {
  indexHtml = indexHtml.replace(
    /<script src="assets\/samples_grade1_5.js">[\s\S]*?<script src="assets\/auth.js"><\/script>/,
    scriptBlock
  );
} else if (!indexHtml.includes('samples_grade6.js')) {
  indexHtml = indexHtml.replace(
    '<script src="assets/sample_viewer.js"></script>',
    `${scriptBlock}`
  );
}

// Add CSS rules to eliminate Layout Shift & smooth rendering
const clsPreventionCSS = `
<style id="cls-performance-fixes">
  /* Containment & Layout Shift Prevention */
  #sample-list {
    min-height: 320px;
    contain: content;
  }
  ...
  #sample-library-modal, #single-sample-modal {
    contain: strict;
    will-change: opacity;
  }
  
  /* Smooth Tab Switches for Single Article View */
  .article-sec-tab {
    padding: 8px 14px;
    border-radius: 14px;
    font-weight: 800;
...
if (!indexHtml.includes('cls-performance-fixes')) {
  indexHtml = indexHtml.replace('</head>', `${clsPreventionCSS}\n</head>`);
}
fs.writeFileSync(indexPath, indexHtml, 'utf8');
console.log("✅ Updated index.html with defer script tags & CLS CSS rules!");


// 2. Overhaul assets/sample_viewer.js with Section Tabs for Single Article Modal
const viewerPath = path.join(__dirname, '..', 'assets', 'sample_viewer.js');

const sectionTabViewerJs = `// ===== SAMPLE EXERCISES VIEWER SYSTEM (PERFORMANCE & MOBILE OPTIMIZED) =====
...
fs.writeFileSync(viewerPath, sectionTabViewerJs, 'utf8');
console.log("✅ Overhauled assets/sample_viewer.js with Mobile Section Tabs System & Zero CLS!");

console.log("🎉 ALL ISSUES FIXED PERFECTLY!");
