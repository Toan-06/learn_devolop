...
console.log("📱 Overhauling Full Mobile UI/UX Responsiveness & Mobile Article Reader...");

// 1. Refine assets/sample_viewer.js for 100% Mobile Responsive Layout
const viewerPath = path.join(__dirname, '..', 'assets', 'sample_viewer.js');

const mobileOptimizedViewerCode = `// ===== SAMPLE EXERCISES VIEWER SYSTEM (MOBILE OPTIMIZED DARK CYBER THEME) =====
...
fs.writeFileSync(viewerPath, mobileOptimizedViewerCode, 'utf8');
console.log("✅ Overhauled assets/sample_viewer.js for full mobile responsiveness!");

// 2. Add Mobile Responsive CSS Helper to index.html if needed
let indexHtml = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
const mobileCSS = `
<style id="mobile-ux-fixes">
  @media (max-width: 768px) {
    /* Touch optimization */
    button, a, input, select, .sample-card, .grade-select-card {
      touch-action: manipulation !important;
      -webkit-tap-highlight-color: transparent !important;
    }
    
    /* Responsive Grade Select Grid */
    #primary-grade-grid {
      grid-template-columns: repeat(auto-fit, minmax(145px, 1fr)) !important;
      gap: 10px !important;
    }

    /* Modal dialog scroll smoothness */
    #sample-library-modal > div, #single-sample-modal > div {
...
if (!indexHtml.includes('mobile-ux-fixes')) {
  indexHtml = indexHtml.replace('</head>', `${mobileCSS}\n</head>`);
  fs.writeFileSync(path.join(__dirname, '..', 'index.html'), indexHtml, 'utf8');
  console.log("✅ Added mobile CSS optimizations to index.html!");
}

console.log("🎉 FULL MOBILE UX OVERHAUL COMPLETED SUCCESSFULLY!");
