...
function getSamplesForGrade(lvl) {
  ...
  if (g === 6)  return window.SAMPLES_GRADE_6  || { math: [], literature: [], english: [], science: [] };
  if (g === 7)  return window.SAMPLES_GRADE_7  || { math: [], literature: [], english: [], science: [] };
  if (g === 8)  return window.SAMPLES_GRADE_8  || { math: [], literature: [], english: [], science: [] };
  if (g === 9)  return window.SAMPLES_GRADE_9  || { math: [], literature: [], english: [], science: [] };
  if (g === 10) return window.SAMPLES_GRADE_10 || { math: [], literature: [], english: [], science: [] };
  if (g === 11) return window.SAMPLES_GRADE_11 || { math: [], literature: [], english: [], science: [] };
  if (g >= 12)  return window.SAMPLES_GRADE_12 || { math: [], literature: [], english: [], science: [] };
  return { math: [], literature: [], english: [], science: [] };
}
    ...
    <div style="background:radial-gradient(circle at 50% 0%, rgba(15,25,55,0.98) 0%, rgba(6,12,30,0.99) 100%);border:1.5px solid rgba(0,242,254,0.35);border-radius:28px;max-width:920px;width:100%;max-height:92vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 25px 70px rgba(0,0,0,.85), 0 0 40px rgba(0,242,254,0.1);animation:scaleUp .3s cubic-bezier(.175,.885,.32,1.275);">
      ...
      <div style="background:linear-gradient(135deg, rgba(0,242,254,0.15) 0%, rgba(124,58,237,0.25) 100%);padding:24px 28px;border-bottom:1px solid rgba(0,242,254,0.2);flex-shrink:0;position:relative;">
        ...
        <div style="color:#00f2fe;font-weight:800;font-size:.8rem;letter-spacing:1px;margin-bottom:6px;display:flex;align-items:center;gap:6px;">
          <span>📚 KHO BÀI GIẢI MẪU CHI TIẾT & BÀI VĂN PHÂN TÍCH HOÀN CHỈNH</span>
        </div>
        <h2 style="color:#f8fafc;margin:0;font-size:1.65rem;font-weight:900;letter-spacing:-0.5px;">Lớp ${lvl} — Kho Bài Giải Chi Tiết Các Môn (${all.length}+ Bài Mẫu)</h2>
        <p style="color:#94a3b8;margin:8px 0 0;font-size:.9rem;">Đề bài đầy đủ • Phân tích chi tiết từng bước & từng từ ngữ • Bài văn mẫu hoàn chỉnh full đoạn</p>
      </div>
      ...
      <div style="background:rgba(10,18,40,0.8);padding:14px 24px;border-bottom:1px solid rgba(0,242,254,0.15);flex-shrink:0;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
        ...
        ${sci.length ? `<button id="tab-sci" onclick="switchSampleTab('sci',${lvl})" style="padding:8px 16px;border-radius:20px;border:1px solid rgba(0,242,254,0.25);font-weight:800;font-size:.82rem;cursor:pointer;background:rgba(15,25,55,0.7);color:#94a3b8;">🔬 KHTN / Lý / Hóa (${sci.length})</button>` : ''}
...
function switchSampleTab(tab, lvl) {
  ...
  ['tab-all','tab-math','tab-lit','tab-eng','tab-sci'].forEach(id => {
